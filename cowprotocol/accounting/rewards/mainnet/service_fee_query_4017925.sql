...
first_event_after_timestamp AS (
  ...
  latest_vouches AS (
    SELECT
      ...
      sender,
      CASE
        WHEN event_type = 'Vouch' THEN True
        ELSE False
      END AS active
    FROM
      (
  ...
  joined_on AS (
    ...
    FROM
      joined_on_data jd
      JOIN ethereum.blocks b ON b.number = jd.evt_block_number
      JOIN bonding_pools bp ON jd.pool = bp.pool
  ),
  named_results AS (
    SELECT
      jd.solver,
    ...
    FROM
      joined_on jd
      JOIN cow_protocol_ethereum.solvers s ON s.address = jd.solver
      JOIN valid_vouches vv ON vv.solver = jd.solver
      AND vv.pool = jd.pool
  ),
...
SELECT
  ...
  END AS expires,
  CASE
    WHEN NOW() > fnr.expires
    AND fnr.pool_name != 'Gnosis' THEN TRUE
    ELSE FALSE
  END AS service_fee
FROM
  filtered_named_results fnr;