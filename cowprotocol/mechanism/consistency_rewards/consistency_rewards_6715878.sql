...
--                            fee_cap * protocol_fee + |penalty| instead of fee_cap * protocol_fee
--
-- Consistency budget:
...
--      consistency_budget = fee_cap * protocol_fee - if(include_penalties, reward, 0)
--        i.e. the full cap when include_penalties = false, or cap + |penalty| when true
--  It is possible that the uncapped reward is negative and that the protocol fee is
...
--  current_reward              - total reward under the current mechanism (wei)
--  new_reward                  - total reward under the proposed mechanism:
--                                performance_reward + consistency_reward (wei)
--  performance_reward          - capped performance reward: min(uncapped_reward, fee_cap *
--                                protocol_fee) per auction, summed over all auctions;
--                                negative rewards (losing auctions) are passed through unchanged (wei)
--  consistency_reward          - share of the weekly consistency budget allocated proportionally
--                                to the solver's consistency metric (wei)
--  metric                      - solver's raw consistency metric value for the week
--  consistency_reward_per_unit - consistency budget divided by total metric across all solvers;
--                                multiply by metric to get each solver's consistency_reward
--  penalty                     - sum of negative rewards (penalties) for the week (wei)
--  protocol_fee                - total protocol fees generated by the solver's settlements (wei)
--  volume                      - total settlement volume (wei)
...
with performance_data_per_auction as (
    select
        ...
        volume,
        protocol_fee,
        least(reward, 0) as penalty,
        case when uncapped_reward <= 0 then reward else least(uncapped_reward, {{fee_cap}} * protocol_fee) end as performance_reward,
        case when uncapped_reward <= 0 then {{fee_cap}} * protocol_fee - if({{include_penalties}}, reward, 0) else greatest({{fee_cap}} * protocol_fee - uncapped_reward, 0) end as consistency_budget
...
select
    ...
    pr.solver,
    current_reward,
    performance_reward + consistency_reward as new_reward,
    performance_reward,
    consistency_reward,
    metric,
    consistency_reward_per_unit,
